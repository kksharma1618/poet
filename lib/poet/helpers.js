...
function createHelpers (poet) {
  ...
  var helpers = {
    ...
    pageURL: function (val) {
      ...
      return utils.createURL(route, val).replace('/page/1', '/').replace('/1', '');
    },
    webpageURL: function(slug) {
        return encodeURI(utils.getRoute(options.routes, 'webpage').replace(':webpage', slug));
    },
    getPostCount: function () { return this.getPosts().length; },
    getPost: function (title) { return poet.posts[title] || false; },
    getPage: function (slug) { return poet.pages[slug] || false; },
    ...
    getPages: function (from, to) {
      var posts = getPages(poet);
      if (from != null && to != null)
        posts = posts.slice(from, to);
    ...
    getPageCount: function (total) {
      total = total || getPosts(poet).length;
      return Math.ceil(total / options.postsPerPage);
    },
    postsWithTag: function (tag, page, all) {
      var posts = getPosts(poet).filter(function (post) {
        return post.tags && ~post.tags.indexOf(tag);
      });

      if(!options.enableTagsPagination || all) {
        return posts;
      }
    ...
    postsWithCategory: function (category, page, all) {
      var posts = getPosts(poet).filter(function (post) {
        return post.category === category;
      });
      if(!options.enableCategoryPagination || all) {
        return posts;
      }
      var from = (page - 1) * options.postsPerPage;
      var to = from + options.postsPerPage - 1;
  ...
  helpers.webpageUrl = helpers.webpageURL;
...
function getPages (poet) {
  if (poet.cache.pages)
    return poet.cache.pages;
  var posts = utils.sortPosts(poet.pages).filter(function (post) {
    // Filter out draft posts if showDrafts is false
    return (poet.options.showDrafts || !post.draft) &&
    // Filter out posts in the future if showFuture is false
      (poet.options.showFuture || post.date < Date.now());
  });
  return poet.cache.pages = posts;
}
