 ...
 * @returns {Array}
 */
var fileModifiedTimes = {}; // storing all modified times in this. against file path
    ...
    return all(files.map(function (file) {
      ...
      return fs.stat(path).then(function (stats) {
        ...
        if(stats.isFile() && stats.mtime) {
            fileModifiedTimes[path] = stats.mtime;
        }
...
exports.getChildrenFiles = getPostPaths; // just an alias. this is used to get any kind of children files from a folder
exports.fileModifiedTimes = fileModifiedTimes;
...
function createPost (filePath, options, isPage) {
  ...
  return fs.readFile(filePath, 'utf-8').then(function (data) {
    ...
    if(isPage) {
        post.url = encodeURI(getRoute(options.routes, 'webpage').replace(':webpage', post.slug));
    }
    post.preview = getPreview(post, body, options);
    post.fileModifiedTime = fileModifiedTimes[filePath] ? fileModifiedTimes[filePath] : false;
...
// same as createPost, we just use options.routes.webpage to create url
function createPage (filePath, options) {
    return createPost(filePath, options, true);
}
...
function getRouteType (route) {
  if(route.indexOf(':category') >= 0 && route.indexOf(':tag') >= 0) {
      return 'categorytag';
  }
  var match = route.match(/\:(post|page|tag|category|webpage)\b/);
