...
function init (poet, callback) {
  var promises = [];
  promises.push(getAllPostsAndPages(poet, true));
  if(fs.existsSync(poet.options.pages)) {
    promises.push(getAllPostsAndPages(poet, false));
  }
...
function createPostsOrPagesFromFiles(poet, isPost, files) {
    var options = poet.options;
    var createPost = isPost ? 'createPost': 'createPage';
        ...
        var post = utils[createPost](file, options).then(function(post) {
            ...
            if (!(post instanceof Error)) {
                if(isPost) {
                    ...
                    return poet.pages[post.slug] = post;
                }
            }
            if(isPost) {
    ...
    return all(collection).then(function (allPosts) {
...
exports.createPostsOrPagesFromFiles = createPostsOrPagesFromFiles;
function getAllPostsAndPages(poet, isPost) {
    var options = poet.options;
    var dir = isPost ? options.posts : options.pages;

    // Get list of files in `options.posts` directory
    var promise = utils.getPostPaths(dir).then(function (files) {
        return createPostsOrPagesFromFiles(poet, isPost, files);
